...
class MaterialHelper:
    ...
    def AddValue(self, inputs: list[Union[str,maxon.GraphNode]] = None, target: list[Union[str,maxon.GraphNode]] = None, mode: Union[str,maxon.Id] = maxon.Id("float")) -> maxon.GraphNode :
        ...
        """
        node = self.helper.AddConnectShader(
        ...
        if isinstance(mode, maxon.Id):            
            self.helper.SetShaderValue(node, "datatype", mode)
        return node
    ...
    def AddUniTransform(self, tex_shader: maxon.GraphNode) -> Optional[maxon.GraphNode]:
        ...
        if not tex_shader:
            return None
        if self.helper.GetShaderId(tex_shader) != "texturesampler":
            raise ValueError("The given node is not a texture shader.")
        
        # The tex value port
        tex_scale = self.helper.GetPort(tex_shader, "com.redshift3d.redshift4c4d.nodes.core.texturesampler.scale")
        tex_offset = self.helper.GetPort(tex_shader, "com.redshift3d.redshift4c4d.nodes.core.texturesampler.offset")
        tex_rotate = self.helper.GetPort(tex_shader, "com.redshift3d.redshift4c4d.nodes.core.texturesampler.rotate")

        # create inner node
        uni_scale_node = self.AddValue()
        self.helper.SetName(uni_scale_node, "UniScale")
        #uni_scale_in = self.helper.GetPort(uni_scale_node, "in")
        uni_scale_out = self.helper.GetPort(uni_scale_node, "out")
        ...
        self.helper.SetName(scale_node, "Scale")
        self.helper.SetName(scale2d_node, "Scale2D")
        self.helper.SetName(offset_node, "Offset")
        self.helper.SetName(rotate_node, "Rotation")
        ...
        self.helper.SetName(groupRoot, "UniTransform")
        ...
        for node in innerNodes:
            if self.helper.GetName(node) == "Scale":
                NodeInner_scale: maxon.GraphNode = node
            if self.helper.GetName(node) == "Offset":
                NodeInner_offset: maxon.GraphNode = node
            if self.helper.GetName(node) == "Rotation":
                NodeInner_rotate: maxon.GraphNode = node
            if self.helper.GetName(node) == "UniScale":
                NodeInner_uniScale: maxon.GraphNode = node
            if self.helper.GetName(node) == "Scale2D":
                NodeInner_Scale2D: maxon.GraphNode = node

        # Find inner node ports
        NodeInnerInput_uni_scale_in = self.helper.GetPort(NodeInner_uniScale, "in")
        NodeInnerInput_offset_in = self.helper.GetPort(NodeInner_offset, "in")
        NodeInnerInput_rotate_in = self.helper.GetPort(NodeInner_rotate, "in")
        NodeInnerInput_scale_in = self.helper.GetPort(NodeInner_scale, "com.redshift3d.redshift4c4d.nodes.core.rsmathmulvector.input2")
        NodeInnerInput_scale2D_in = self.helper.GetPort(NodeInner_Scale2D, "in")
        # Find outer node ports
        NodeInnerOutput_scale_out = self.helper.GetPort(NodeInner_scale, "com.redshift3d.redshift4c4d.nodes.core.rsmathmulvector.out")
        NodeInnerOutput_offset_out = self.helper.GetPort(NodeInner_offset, "out")
        NodeInnerOutput_rotate_out = self.helper.GetPort(NodeInner_rotate, "out")

        # Connect scale
        self.helper.GetPort(NodeInner_Scale2D, "out").Connect(NodeInnerInput_scale_in)
        ...
        # Hide input ports
        self.helper.RemovePort(groupRoot, groupPortIn_scale)
        self.helper.RemovePort(groupRoot, groupPortIn_unify)
        self.helper.RemovePort(groupRoot, groupPortIn_offset)
        self.helper.RemovePort(groupRoot, groupPortIn_rotate)
