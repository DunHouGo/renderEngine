...
class NodeGraghHelper:
    ...
    def __init__(self, material: c4d.BaseMaterial, **kwargs):
        """
        ...
        if self.material:
            ...
            if "nodespaceId" in kwargs:
                self.nodespaceId: maxon.Id = kwargs["nodespaceId"]

            else:
                for nid in SUPPORTED_NODESPACES:
                    if self.nodeMaterial.HasSpace(nid):
                        self.nodespaceId = maxon.Id(nid)
                        break

            if self.nodespaceId is None:
                self.nodespaceId = c4d.GetActiveNodeSpaceId()
    ...
    def MaxonToC4D(self, data: maxon.data) -> Any:
        if isinstance(data, maxon.data):
            if isinstance(data, maxon.Color):
                return c4d.Vector(data.r, data.g, data.b)
    ...
    def IsConnected(self, NodeorPort: maxon.GraphNode, port: Union[maxon.GraphNode,str] = None) -> bool:
        ...
        if isinstance(NodeorPort, maxon.GraphNode) and port is None:
            if self.IsPort(NodeorPort):
                return self.IsPortConnected(NodeorPort)
            elif self.IsNode(NodeorPort):
                return self.IsNodeConnected(NodeorPort)
            return False
    ...
    # New 获取关联的BaseList2D对象
    def GetBaseList2D(self, node: maxon.GraphNode) -> Optional[c4d.BaseList2D]:
        """
        Get the BaseList2D of the node

        Args:
            node (maxon.GraphNode): the node

        Returns:
            Optional[c4d.BaseList2D]: the link BaseList2D
        """
        if self.IsPort(node):
            # For each GraphNode (node and port) a BaseList2D is created.
            parentNode: maxon.GraphNode = node.GetAncestor(maxon.NODE_KIND.NODE)
            parentNodePath = parentNode.GetPath()
        elif self.IsNode(node):
            parentNodePath = node.GetPath()
        # Retrieve the BaseList2D object corresponding to the True Node and NOT the port.
        return self.nodeMaterial.GetBaseListForNode(self.nodespaceId, parentNodePath)

    # New 获取对应文档
    def GetDocument(self, node: maxon.GraphNode) -> Optional[c4d.documents.BaseDocument]:
        """
        Get the BaseDocument of the node

        Args:
            node (maxon.GraphNode): the node

        Returns:
            Optional[c4d.BaseList2D]: the link BaseDocument
        """
        bs2d: Optional[c4d.BaseList2D] = self.GetBaseList2D(node)
        if bs2d:
            return bs2d.GetDocument()

    # New 获取对应材质
    def GetMaterial(self, node: maxon.GraphNode) -> Optional[c4d.BaseMaterial]:
        """
        Get the BaseMaterial of the node

        Args:
            node (maxon.GraphNode): the node

        Returns:
            Optional[c4d.BaseList2D]: the link BaseMaterial of the node
        """
        bs2d: Optional[c4d.BaseList2D] = self.GetBaseList2D(node)
        if bs2d:
            return bs2d.GetMain()
    ...
    # New 获取对应DescID   
    def GetDescID(self, node: maxon.GraphNode) -> Optional[c4d.DescID]:
        """
        Get the DescID of the node

        Args:
            node (maxon.GraphNode): the node

        Returns:
            Optional[c4d.BaseList2D]: the link DescID
        """
        if not isinstance(node,maxon.GraphNode) :
            raise ValueError(f'{sys._getframe().f_code.co_name} Expected GraphNode, got {type(node)}')
        return self.nimbusRef.GetDescID(node.GetPath())

    #=============================================
    # Animation (May be modified)
    #=============================================

    # New 获取CTrack
    def GetPortCtrack(self, node: maxon.GraphNode, pushLevel: c4d.DescLevel = None) -> Optional[c4d.CTrack]:
        """
        Get the CTrack of the node

        Args:
            node (maxon.GraphNode): the node
            pushLevel (c4d.DescLevel) : if the channel has components, we should push a c4d.DescLevel to find the lowest channel

        Returns:
            Optional[c4d.BaseList2D]: the link CTrack
        """
        descID = self.GetDescID(node)
        if pushLevel:
            descID.PushId(pushLevel)
        return self.GetBaseList2D(node).FindCTrack(descID)

    # New 添加CTrack
    def SetPortCtrack(self, node: maxon.GraphNode, pushLevel: c4d.DescLevel = None) -> Optional[c4d.CTrack]:
        """
        Get the CTrack of the node
        ...
        Returns:
            Optional[c4d.BaseList2D]: the link CTrack
        """
        if (track:=self.GetPortCtrack(node, pushLevel)) is None:
            doc = self.GetDocument(node)
            obj = self.GetBaseList2D(node)
            descID = self.GetDescID(node)
            track: c4d.CTrack = c4d.CTrack(obj, descID)
            if track is None:
    ...
    # New 同于SetPortCtrack
    def AddTrack(self, node: maxon.GraphNode, pushLevel: c4d.DescLevel = None) -> Optional[c4d.CTrack]:
        """
        Get the CTrack of the node
        ...
        Returns:
            Optional[c4d.BaseList2D]: the link CTrack
        """
        return self.SetPortCtrack(node, pushLevel)

    # New 删除Ctrack
    def RemoveTrack(self, port: maxon.GraphNode) -> bool:
        doc: c4d.documents.BaseDocument = self.GetDocument()
        if not doc:
            return False
        track = self.GetPortCtrack(port)
        if track is None:
            return False
        doc.AddUndo(c4d.UNDOTYPE_DELETEOBJ, track)
        track.Remove()
        return True
    
    # New 删除CKey
    def RemoveKey(self, port: maxon.GraphNode):
        doc = self.GetDocument(port)
        if not doc:
            return False
        track = self.GetPortCtrack(port)
        if track is None:
        ...
        for i in range(curve.GetKeyCount()):
            ckey = curve.GetKey(i)
            if ckey.GetTime().GetFrame(doc.GetFps()) == doc.GetTime().GetFrame(doc.GetFps()):
                doc.AddUndo(c4d.UNDOTYPE_DELETEOBJ, ckey)
                ckey.Remove()
                return True

    # New 添加CKey
    def AddKey(self, port: maxon.GraphNode, time: c4d.BaseTime=None, value: Any=None, linear: bool=True):
        doc = self.GetDocument(port)
        track = self.GetPortCtrack(port)
        obj = self.GetBaseList2D(port)
        if track is None:
            track = c4d.CTrack(self.GetBaseList2D(port), self.GetDescID(port))
            if track is None:
                raise MemoryError("Could not allocate track.")
            obj.InsertTrackSorted(track)
            doc.AddUndo(c4d.UNDOTYPE_NEWOBJ, track)
        ...
        if not time:
            ...
            time = c4d.BaseTime(currentFrame, doc.GetFps())
        
        if not value:
            value = self.GetPortData(port)

        key.SetValue(curve, value)
        key.SetTime(curve, time)
        
        curve.InsertKey(key)
        doc.AddUndo(c4d.UNDOTYPE_NEWOBJ, key)
    ...
    def GetNodes(self, shader: Union[maxon.GraphNode, str]) -> list[maxon.GraphNode]:
        ...
        if isinstance(shader, (str, maxon.Id, maxon.data.Id)):
            asset_id = shader
    ...
    # 获取Port的Id字符串 ==> ok
    def GetPortId(self, port: maxon.GraphNode) -> str:
        if not self.IsPort(port):
            raise ValueError(f'{sys._getframe().f_code.co_name} Expected a port, got {type(port)}')
    ...
    def GetPort(self, shader: maxon.GraphNode, port_id :str = None) -> Union[maxon.GraphNode,bool]:
        ...
        out_ids = ['outcolor','output','out', 'output.default']
        ...
        if self.nodespaceId == VR_NODESPACE:
            if port_id == None:
                ...
                port: maxon.GraphNode = shader.GetInputs().FindChild(port_id)
                # if port.IsNullValue():
                    #port_id_tmp = f"{self.GetAssetId(shader)}.uvwgen"
                    #port = shader.GetInputs().FindChild(port_id_tmp)
                if port.IsNullValue():
                    port = shader.GetOutputs().FindChild(port_id)
                if port.IsNullValue():
                    return False
    ...
    def IsPortConnected(self, port: maxon.GraphNode) -> bool:
        ...
        #         return True
        # return False
        if len(self.GetConnectedPorts(port)) != 0:
            return True
        return False
    ...
    # NEW 获取连接的节点  ==> ok
    def GetConnectedPorts(self, port: maxon.GraphNode) -> list[maxon.GraphNode]:
        """
