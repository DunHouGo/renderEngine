...
class MaterialHelper(NodeGraghHelper):
    ...
    # 创建Math value ==> OK
    def AddMathvalue(self, inputs: list[Union[str,maxon.GraphNode]] = None, target: list[Union[str,maxon.GraphNode]] = None, dataType: int = 3) -> maxon.GraphNode :
        """
        Adds a new value shader to the graph.
        dataType: 0:int, 1:flaot, 2:vector, 3:color, 4:string

        """
        shader =  self.AddConnectShader(
        ...
        self.SetPortData(self.GetPort(shader, "data_type"), dataType)
        return shader
    ...
    def AddMathMix(self, inputs: list[Union[str,maxon.GraphNode]] = None, target: list[Union[str,maxon.GraphNode]] = None) -> maxon.GraphNode :
        """
        Adds a new Mix shader to the graph.

        """
        return self.AddConnectShader(
            nodeID ="com.autodesk.arnold.shader.mix_rgba",
            input_ports = ['input1','input2'],
            connect_inNodes = inputs,
            output_ports=['output'], 
            connect_outNodes = target
    ...
    # 创建Shader Rgba ==> OK
    def AddShaderRgba(self, inputs: list[Union[str,maxon.GraphNode]] = None, target: list[Union[str,maxon.GraphNode]] = None) -> maxon.GraphNode :
        """
        Adds a new Layer Rgba shader to the graph.

        """
        return self.AddConnectShader(
            nodeID ="com.autodesk.arnold.shader.layer_shader",
            input_ports = ['input1','input2','input3','input4','input5','input6','input7','input8'],
            connect_inNodes = inputs,
            output_ports=['output'], 
            connect_outNodes = target
    ...
    def AddLayerRgba(self, inputs: list[Union[str,maxon.GraphNode]] = None, target: list[Union[str,maxon.GraphNode]] = None) -> maxon.GraphNode :
        ...
        """
        return self.AddConnectShader(
            nodeID ="com.autodesk.arnold.shader.layer_rgba",
            input_ports = ['input1','input2','input3','input4','input5','input6','input7','input8'],
            connect_inNodes = inputs,
            output_ports=['output'], 
            connect_outNodes = target
